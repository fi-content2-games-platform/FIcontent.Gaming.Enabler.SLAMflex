...
public:
  ...
  std::string GetMessageForUser();
    ...
    void SetSendMessageToUnity(SendMessage sm);  	//Used for setting one way communication between Tracker and Unity3D
...
protected:
  ...
  inline float RadiansToDegrees(float r) { return r*(180/M_PI); }
  ...
  SendMessage sendToUnity;		//Used to send message to Unity
  int threshold;				//Threshold for fast corner detection
  bool thresholdReached;		//If threshold is reached stop automatic threshold adjustment
  DetectionState currentState;  //Current state of tracking
