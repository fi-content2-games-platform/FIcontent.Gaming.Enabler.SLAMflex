...
int KeyFrame::MakeKeyFrame_Lite(BasicImage<byte> &im, int threshold)
{
  ...
  for(int i=0; i<LEVELS; i++)
	...
	{  // .. make a half-size image from the previous level..
      ...
      if(i == 0)
      {
    	  fast_corner_detect_10(lev.im, lev.vCorners, threshold);
    	  retVal = lev.vCorners.size();
      }
      if(i == 1)
    	  fast_corner_detect_10(lev.im, lev.vCorners, threshold+5);
      if(i == 2)
    	  fast_corner_detect_10(lev.im, lev.vCorners, threshold+5);
      if(i == 3)
    	  fast_corner_detect_10(lev.im, lev.vCorners, threshold);
