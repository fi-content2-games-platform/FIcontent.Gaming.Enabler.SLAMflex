...
void MapMaker::Reset()
{
  ...
  counterForConvergenceLimit = CounterForConvergenceLimit;
}


void MapMaker::SetSendMessageToUnity(SendMessage sm)
{
	sendStringToUnity = sm;
}
...
MapMaker::~MapMaker()
{
  ...
  LOGV(LOG_TAG, "Info: %s", "Waiting for mapmaker to die..");
  join();
  cout << " .. mapmaker has died." << endl;
  LOGV(LOG_TAG, "Info: %s", ".. mapmaker has died.");
}
			      ...
			      SE3<> &se3TrackerPose)
{
  ...
  if(!bGood)
    {
      ...
      LOGV(LOG_TAG, "Info: %s", "Could not init from stereo pair, try again.");
      return false;
    }
  ...
  if(dTransMagn == 0)
    {
      ...
      LOGV(LOG_TAG, "Info: %s", "  Estimated zero baseline from stereo pair, try again.");
      return false;
    }
  ...
  *pkSecond = kS;
  ...
  counterForConvergence = 0;
  while(!mbBundleConverged_Full)
    {
      if (counterForConvergence < counterForConvergenceLimit)
	  {
    	  counterForConvergence++;
	  }
      else
      {
    	  return false;
	  ...
	  BundleAdjustAll();
      LOGV(LOG_TAG, "Info: mbBundleConverged_Full, counter:  %i", counterForConvergence);
      if(mbResetRequested)
	return false;
    }
  ...
  LOGV(LOG_TAG, " MapMaker: made initial map with: %i", mMap.vpPoints.size());
  ostringstream tmp;
  tmp << "MapMaker: made initial map with "<<mMap.vpPoints.size()<<" points."<< endl;
  sendStringToUnity.SendStringToUnity(tmp.str().c_str());
