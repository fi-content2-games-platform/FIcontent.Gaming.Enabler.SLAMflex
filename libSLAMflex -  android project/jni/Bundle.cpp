...
int Bundle::Compute(bool *pbAbortSignal)
{
  ...
  while(!mbConverged  && !mbHitMaxIterations && !*pbAbortSignal)
	...
	bNoError = Do_LM_Step<Cauchy>(pbAbortSignal, &lastMdSigmaSquared);
      else if(gvsMEstimator == "Tukey")
	bNoError = Do_LM_Step<Tukey>(pbAbortSignal, &lastMdSigmaSquared);
      else if(gvsMEstimator == "Huber")
	bNoError = Do_LM_Step<Huber>(pbAbortSignal, &lastMdSigmaSquared);
      else
	{
  ...
  if(mbHitMaxIterations)
  {
    ...
    LOGV(LOG_TAG, "Info: %s", " Hit max iterations.");
  }
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal, double *lastMdSigmaSquared)
{
...
//	    };
      ...
      LOGV(LOG_TAG, "Info: dSumSquaredUpdate: %g lastMdSigmaSquared: %g, difference: %g",  dSumSquaredUpdate, *lastMdSigmaSquared, dSumSquaredUpdate - (*lastMdSigmaSquared));
      *lastMdSigmaSquared = dSumSquaredUpdate;
